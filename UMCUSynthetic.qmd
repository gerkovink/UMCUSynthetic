...
In this workshop, you will (at least) use the packages `mice` [@mice], `ggmice` [make sure to download the latest version from GitHub, @ggmice], `magrittr` [@magrittr] and `psych` [@psych]. Make sure to load them (in case you haven't installed them already, install them first, using `install.packages("package.name")`).
